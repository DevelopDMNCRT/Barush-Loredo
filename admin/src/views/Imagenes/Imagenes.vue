<template>
  ...
  <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:gap-6">
      ...
      >
        ...
        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <button @click.stop="deleteImage(img.id)" class="bg-meta-1 text-white p-3 rounded-full hover:bg-opacity-90 transition-all transform scale-90 group-hover:scale-100 shadow-lg" title="Eliminar del slider">
    ...
    <div v-if="selectedPreviewImage" class="fixed inset-0 z-[9999999] flex items-center justify-center bg-black/80 backdrop-blur-md p-4" @click="selectedPreviewImage = null">
      <div class="relative max-w-5xl max-h-[90vh] rounded-lg overflow-hidden" @click.stop>
        <button @click="selectedPreviewImage = null" class="absolute top-3 right-3 bg-black/60 text-white rounded-full p-2 hover:bg-black transition-colors z-10">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <img :src="selectedPreviewImage" class="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl" />
      </div>
    </div>
